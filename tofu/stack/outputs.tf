...
output "ssh_command" {
  ...
  value       = "cloudflared access ssh --hostname ssh-jose-perezgonzalez.nona.company"
}
...
output "ssh_config" {
  ...
  value       = <<-EOT
    Host nexus
      HostName ssh-jose-perezgonzalez.nona.company
      User root
...
output "service_urls" {
  ...
  value = {
    ...
    key => "https://${service.subdomain}-jose-perezgonzalez.nona.company"
  }
}
...
output "secrets" {
  ...
  value = {
    ...
    redpanda_kafka_public_url           = "redpanda-kafka-jose-perezgonzalez.nona.company:9092"
    redpanda_schema_registry_public_url = "http://redpanda-schema-registry-jose-perezgonzalez.nona.company:18081"
